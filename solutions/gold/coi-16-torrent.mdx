...
(Analysis by Rishikesh Anabathula)

First, let's solve the problem using only 1 starting node instead of 2. 
We can solve this using tree dp. Let the starting node be the root and 
$dp_i$ be the time it takes for node $i$ to populate it's entire 
subtree. If $C$ are the children of $i$, we have the recurrence 

<center>
$dp_i=\max\limits_{1\le i \le |C|}(dp_{C_i}+i)$
</center>

Since we are trying to minimize this, we can greedily assign the largest 
$dp_{C_i}$ to $i=1$ and smallest $dp_{C_i}$ to $i=|C|$, and take the 
maximum. Our final answer is the dp value of the root.

Let $path_i$ be the $i$'th node on the path from node $a$ to node $b$.
Notice that some of the values on this path will be populated using
node $a$, and the rest using node $b$. This means there exists a value
$k$ where $path_1,\dots path_k$ is populated by node $a$, and
$path_{k+1},\dots path_{|path|}$ is populated by node $b$. This means
that we can split the tree into 2 seperate parts, where the first part is
populated by node $a$ and the second part is populated by node $b$.
These 2 subtrees are independent, so our answer is the maximum answer
from each subtree.

We can see that as $k$ increases, the answer for the first tree is
non-decreasing and the answer for the second tree is non-increasing.
This means we can ternary search on the value of $k$.
